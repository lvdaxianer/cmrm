...
一个用于解放 AI 工具模型注册管理的命令行工具，可以快速切换 Claude CLI 工具的模型配置。

## 项目意图

这个插件的目的是**解放 AI 工具的模型注册管理**。它提供了一种便捷的方式来管理 Claude CLI 工具的模型配置，无需手动编辑配置文件。
...
选中配置将：
- 合并写入 Claude 配置文件（保留现有字段）
- 在 `~/.claude/.cmrm/` 目录创建备份

### 添加新模型

使用 `/add-model` 命令输入配置字段：
...
### 0.0.3
- 🗑️ 移除 OpenCode 支持（专注于 Claude CLI）
- 🔄 简化架构（单一工具专注）
- ✅ 更简洁的命令流程（无需工具选择步骤）
